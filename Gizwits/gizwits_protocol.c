...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagkey)
    {
        info->event[info->num] = EVENT_key;
        info->num++;
        dataPoints->valuekey = gizStandardDecompressionValue(key_BYTEOFFSET,key_BITOFFSET,key_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    {
        //GIZWITS_LOG("valuekey Changed\n");
        ret = 1;
    }

    if(last->valueopen_num != cur->valueopen_num)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            //GIZWITS_LOG("valueopen_num Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueharm_gas != cur->valueharm_gas)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            //GIZWITS_LOG("valueharm_gas Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueopen_num = gizY2X(open_num_RATIO,  open_num_ADDITION, dataPoints->valueopen_num); 

    devStatusPtr->valueharm_gas = exchangeBytes(gizY2X(harm_gas_RATIO,  harm_gas_ADDITION, dataPoints->valueharm_gas)); 
