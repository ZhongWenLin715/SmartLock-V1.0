...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *gizdata, uint32_t len)
{
  ...
  for(i=0; i<info->num; i++)
  {
    ...
    {
      case EVENT_key:
        currentDataPoint.valuekey = dataPointPtr->valuekey;
        //GIZWITS_LOG("Evt: EVENT_key %d \n", currentDataPoint.valuekey);
        if(0x01 == currentDataPoint.valuekey)
        {
              OLED_Clear();
              delay_ms(200);
...
void userHandle(void)
{
   ...
   // currentDataPoint.valueopen_num = ;//Add Sensor Data Collection
    currentDataPoint.valueharm_gas = Get_Adc_Average(ADC_Channel_9,10);
...
void userInit(void)
{
    ...
    /*
      currentDataPoint.valuekey = ;
      currentDataPoint.valueopen_num = ;
      currentDataPoint.valueharm_gas = ;
    */
